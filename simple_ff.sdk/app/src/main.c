...
int main() {
	...
	// Initialize drivers. The header files that correspond to each driver provide more
	// information on how to utilize each driver. More information can be found in the
	// user guides (the user guides are incredibly useful for debugging).
	{
		int Status;
...
void spi_handler(void *CallBackRef, u32 StatusEvent, unsigned int ByteCount) {
	...
	current_buffer->samples[sample_total++] = input_sample<<SAMPLE_INPUT_SHIFT;
